...
4. Copy from `CANONICAL`: `coreQuestion` → `summary` (capability voice), `changeNature`, `identity.status` / `specUrl` / `specDate` / `testReleaseUrl`, keywords, comparison. Do not re-resolve pins from a local `ethereum/EIPs` checkout.
